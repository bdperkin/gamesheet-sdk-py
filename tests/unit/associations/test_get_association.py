...
def test_get_association_returns_single_association(config: Config) -> None:
    ...
    association_id = "101"
    get_endpoint = f"{TEST_BASE_URL}/api/associations/{association_id}"
    responses.add(
        responses.GET,
        get_endpoint,
        json={
    ...
    with Session(config) as session:
        ...
        result = get_association(session, association_id)

    assert result.id == association_id
...
def test_get_association_sends_bearer_and_jsonapi_accept(config: Config) -> None:
    ...
    with Session(config) as session:
        ...
        get_association(session, association_id)
