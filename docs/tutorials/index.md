...
- **Don't explain _why_.** That's the job of {doc}`../explanation/index`. A tutorial shows that something works; the explanation says why it has to.
- **Lead the reader to a concrete, working result** — something they can run, see, or share.
- **Avoid choices.** Pick one path and walk it. Alternatives belong in {doc}`../how-to/index`.
