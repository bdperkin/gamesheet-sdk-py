...
class AuthenticatedSession(Session):
    ...
    def __init__(
        ...
        *,
        ...
        Constructs a :class:`~gamesheet_sdk.common.session.Session` with the given configuration, stores the
        refresh token for automatic renewal, and attaches the access token as the bearer. Optionally
        registers a callback to persist updated tokens after a successful refresh.
    ...
    def _notify_refresh(self: AuthenticatedSession, new_tokens: dict[str, str]) -> None:
    ...
    def _try_refresh(self: AuthenticatedSession) -> bool:
    ...
    def request(
        ...
        **kwargs: Any,
        ...
        Performs the request using the parent :class:`~gamesheet_sdk.common.session.Session.request` method.
        If the response status is 401 Unauthorized or 403 Forbidden, attempts to refresh the access token
        using the stored refresh token, updates the bearer token, invokes the ``on_refresh`` callback if
        provided, and retries the original request exactly once. Both 401 and 403 are treated as
        authentication failures because different GameSheet API endpoints use different status codes for
        expired tokens.
