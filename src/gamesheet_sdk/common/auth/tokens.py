...
The token storage mechanism uses a browser storage state file (JSON) that mirrors the structure of
Playwright's persistent context storage. This allows tokens to be shared between headless browser sessions and
direct HTTP sessions.
...
def load_access_token(config: Config) -> str | None:
    ...
    ``None`` if the storage state file is missing, unreadable, or does not contain a token. The access token
    is used for authenticating API requests and typically has a short lifetime (10 minutes).
...
def load_refresh_token(config: Config) -> str | None:
    ...
    Companion to :func:`load_access_token`. Returns the value of the ``refreshToken`` localStorage entry. Used
    to drive :func:`refresh_access_token` and
...
def build_token_updates(
    *,
    ...
    Creates a dictionary mapping localStorage keys to token values, including only the tokens that were
    provided (non-``None``). This is used internally by :func:`save_tokens` to prepare the updates for browser
    storage.
...
def save_tokens(
    ...
    *,
    ...
    :class:`~gamesheet_sdk.common.browser.BrowserSession` and direct token loading use this same storage
    ...
    **Example: **
    ...
    Args:
        config (Config): Configuration object containing the base URL and browser state path where tokens will
            be saved.
...
def refresh_access_token(
    ...
    *,
    ...
    POSTs to :data:`~gamesheet_sdk.common.auth.constants.REFRESH_URL` with ``Authorization: Bearer
    <refresh_token>`` and an empty JSON body. The gateway returns a new access token (10-min TTL), a new
    refresh token (long TTL, replaces the one you sent), and a roles token.

    This is a standalone HTTP call that does not use a :class:`~requests.Session`, so it can be used from
    inside :class:`~gamesheet_sdk.common.auth.session.AuthenticatedSession`'s retry path without recursing.
    ...
    **Example: **
    ...
    Args:
        ...
        timeout (float): Request timeout in seconds. Defaults to
            :data:`~gamesheet_sdk.common.auth.constants.REFRESH_TIMEOUT_ S`.

    Returns:
        dict[str, str]: Dictionary with keys ``access``, ``refresh``, and ``roles``, each containing the
