...
class Config(BaseSettings, env_prefix="GAMESHEET_", extra="ignore"):
    ...
    Attributes:
        base_url (str): Root URL of the GameSheet WebUI (the dashboard
            app)
        username (str | None): GameSheet account username/email
        password (SecretStr | None): GameSheet account password (stored
            as SecretStr)
        session_path (Path): Where to persist cookie state between runs
        timeout (float): Default per-request HTTP timeout in seconds
            (must be > 0)
        user_agent (str | None): Override the default User-Agent header
            sent by the Session
        verify_ssl (bool): Whether to verify TLS certificates on
            outgoing requests
        request_retries (int): Automatic retries on 5xx responses and
            connection errors
        browser_state_path (Path): Where to persist Playwright storage
            state between runs
        browser_headless (bool): Launch the Playwright browser in
            headless mode
    **Examples:**
