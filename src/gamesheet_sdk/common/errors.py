...
from gamesheet_sdk.common.shared.constants import HELP_USE_SEASONS_LIST

# Authentication error messages
ERROR_MSG_401_GENERIC = (
    "Access token rejected (HTTP 401) for {context}. Use `gamesheet-admin login` to authenticate."
)

ERROR_MSG_401_EXPIRED = (
    "Access token rejected (HTTP 401). Likely expired; "
    "re-run `gamesheet-admin login` to refresh and try again."
)

ERROR_MSG_403_GENERIC = (
    "Access forbidden (HTTP 403) for {context}. "
    "Your session cookies may have expired. "
    "Use `gamesheet-admin login` to re-authenticate."
)

ERROR_MSG_NO_SESSION = "No saved session found. Run `gamesheet-admin login` first."

ERROR_MSG_REFRESH_REJECTED = "Refresh token rejected. Run `gamesheet-admin login` to re-authenticate."
...
ERROR_MSG_404_LEAGUE = (
    ...
    "To get valid league IDs, run: gamesheet-admin leagues list"
)

ERROR_MSG_404_TEAM = (
    "Team '{team_id}' not found (HTTP 404). "
    "Make sure you're using a valid team ID. "
    "To get valid team IDs, run: gamesheet-admin teams list --season-id {season_id}"
)
...
ERROR_MSG_BROADCASTER_INVALID = (
    ...
    "Use 'gamesheet-admin games broadcasters list' to see valid options."
)
