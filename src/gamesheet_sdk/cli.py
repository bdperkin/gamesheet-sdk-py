...
class ResourceGroup(click.Group):
    ...
    **Default sub-command.** Pass ``default="list"`` and a bare invocation of the group implicitly runs
    ...
    def parse_args(self, ctx: click.Context, args: list[str]) -> list[str]:
        ...
        of click's parsing machinery treats it exactly like an explicit call.

        Skip the injection when click is parsing for shell completion (``resilient_parsing=True``). Otherwise
        click's completion walker would silently descend into the default sub-command, and a bare ``gamesheet-
        sdk-py associations <TAB>`` would yield the leaf command's options instead of the group's verbs.
        """
...
def confirm_destructive(target: str = "this resource") -> Callable[[F], F]:
    """Gate a click command with an interactive ``[y/N]`` confirmation.

    Adds a ``--force/-f`` flag to the wrapped command. The flag skips the prompt (suitable for automation/CI);
    without it the command aborts unless the user confirms interactively. Intended for use on any
    ``delete``/``rm``/``remove`` sub-command across resource groups.
    """
...
def _should_color(handler: logging.StreamHandler[Any]) -> bool:
    ...
    return bool(stream is not None and getattr(stream, "isatty", bool)())
...
def cli(
    ...
    *,
    ...
    """Unofficial CLI for the GameSheet Inc.
...
def completion_command(shell: str) -> None:
    r"""Print a SHELL completion script to stdout.

    SHELL must be one of ``bash``, ``zsh``, ``fish``. The emitted script teaches the shell to tab-complete
    sub-commands (including aliases like ``ls`` for ``list``), option names, and choice values such as
    ``--format``.

    \b Bash (current session):     eval "$(gamesheet-sdk-py completion bash)"

    \b Bash (permanent):     gamesheet-sdk-py completion bash >> ~/.bashrc

    \b Zsh (permanent):     gamesheet-sdk-py completion zsh >> ~/.zshrc

    \b Fish (permanent — fish auto-loads ~/.config/fish/completions/):     gamesheet- sdk-py completion fish >
    ~/.config/fish/completions/gamesheet-sdk-py.fish
    """
    ...
    if comp_cls is None:  # pragma: no cover — click registers all three above
        _err_msg = f"No completion class registered for shell '{shell}'."
        raise click.ClickException(_err_msg)
...
def _list_associations_or_exit(session: AuthenticatedSession) -> list[Any]:
    ...
    try:
        with session:
            return _list_associations_action(session)
    except AuthenticationError as exc:
...
def seasons_group() -> None:
    """Manage GameSheet seasons.

    Invoking ``seasons`` with no sub-command runs ``list`` by default.
    """


def _list_seasons_or_exit(session: AuthenticatedSession, league_id: str) -> list[Any]:
    """Run the list_seasons action, mapping known errors to a red message + ``Exit(1)``."""
    try:
        with session:
...
def seasons_list_command(
    ...
    config: Config = ctx.obj
    session = _build_associations_session(ctx, config)
    seasons = _list_seasons_or_exit(session, league_id)
...
def main(argv: list[str] | None = None) -> int:
    ...
    control-flow exceptions are converted to integer returns     rather than ``sys.exit()`` calls, preserving
    the original ``main(argv) -> int`` contract.
    """
    try:
