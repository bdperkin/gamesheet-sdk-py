...
def divisions_update_command(
    ...
    At least one of --title or --external-id must be provided.
    Requires authentication (run 'gamesheet-sdk-py login' first).
    """
    ...
    def _update_with_kwargs(sess: AuthenticatedSession, div_id: str) -> Division:  # type: ignore[name-defined]
        return _update_division_action(sess, div_id, title=title, external_id=external_id)

    division = run_action_or_exit(session, _update_with_kwargs, division_id)
    rows = [division.model_dump(mode="json")]
    rendered = render(rows, fmt=output_format)
    write_output(rendered, output_path, fmt=output_format)
    if output_path is None:
